...
function askConfirmation({title='Confirmar acción',message='¿Deseas continuar?',accept='Aceptar',icon='⚠️'}={}){return new Promise(resolve=>{const modal=$('confirmModal'),cancel=$('confirmCancel'),ok=$('confirmAccept');$('confirmTitle').textContent=title;$('confirmMessage').textContent=message;$('confirmIcon').textContent=icon;ok.textContent=accept;modal.classList.add('show');modal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';let done=false;const close=value=>{if(done)return;done=true;modal.classList.remove('show');modal.setAttribute('aria-hidden','true');document.body.style.overflow='';cancel.onclick=null;ok.onclick=null;resolve(value)};cancel.onclick=()=>close(false);ok.onclick=()=>close(true);modal.onclick=e=>{if(e.target===modal)close(false)}})}
...
async function returnToRoomLobby(){
 const active=state?.phase==='countdown'||state?.phase==='playing';
 const ok=await askConfirmation({title:active?'¿Volver al lobby sin salir?':'¿Volver al lobby?',message:active?'Seguirás perteneciendo a la sala.\nTus cartillas y tu participación se conservarán. Podrás regresar a esta misma partida.':'Tu sesión en la sala se conservará.',accept:'Volver al lobby',icon:'↩️'});
 if(!ok)return;
 stopNarration(false);
 try{await api('/api/action',{code:room,playerId,type:'leaveToLobby'})}catch(e){return toast(e.message)}
 toast('Volviste al lobby. Puedes regresar a la partida.');
}
async function resumeCurrentGame(){
 try{await api('/api/action',{code:room,playerId,type:'setView',inGameView:true});toast('Regresaste a la partida')}catch(e){toast(e.message)}
}
async function leaveRoomCompletely(){
 const active=state?.phase==='countdown'||state?.phase==='playing';
 const ok=await askConfirmation({title:active?'¿Abandonar definitivamente?':'¿Salir completamente de la sala?',message:active?'La partida ya comenzó.\nSi abandonas perderás tu participación y no podrás regresar a esta ronda.\nTus cartillas quedarán liberadas.':'Saldrás de la sala y se liberarán tus cartillas. Podrás volver solo si la partida aún no comenzó.',accept:'Sí, abandonar',icon:'⚠️'});
 if(!ok)return;
 saveProfile();stopNarration(true);AudioEngine.stopMusic();
 try{await api('/api/action',{code:room,playerId,type:'abandon'})}catch(e){return toast(e.message)}
...
$('leaveLobby').onclick=leaveRoomCompletely;$('returnLobbyGame').onclick=returnToRoomLobby;$('leaveGame').onclick=leaveRoomCompletely;$('leaveResult').onclick=returnToRoomLobby;$('resumeGame').onclick=resumeCurrentGame;
